...
export class BoardComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this._gameService.boardHasChanded$.subscribe((boardData: BoardStructure) => {
	...
	createNewEmptyBoard(): void {
		this._gameService.newEmptyBoard(this.vertical, this.horizontal, this._minesLength);
	...
	private _isThereCellAround(cellAroundCoords: number[]): boolean {
		return cellAroundCoords[0] >= 0 && cellAroundCoords[0] < this._gameService.vertical &&
			cellAroundCoords[1] >= 0 && cellAroundCoords[1] < this._gameService.horizontal;
	}
	...
	private _updateRemainingEmptyCells(): void {
		...
		const remainEmptyCells = gameService.vertical * gameService.horizontal - (gameService.minesLenght + allOpenedCells.length);
